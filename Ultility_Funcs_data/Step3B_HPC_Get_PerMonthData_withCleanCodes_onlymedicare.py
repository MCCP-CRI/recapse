...
def Step3B_HPC_Get_PerMonthData_withCleanCodes_onlymedicare(user_name, path_input, path_output, mecareClaims, drug_code):
    ...
    ID_df = ID_df.drop(ID_df[(ID_df["in_Medicare"] == 0) & (ID_df["in_Medicaid"] == 1)].index)
    ...
    for i in range(len(analysis_ID)): #len(analysis_ID)
        ...
        #if medicare_df != None:
        ICD_diag_cols2 = ['DGNS_CD{}'.format(j) for j in range(1, 25+1)]  #ICD9 or ICD10
        ...
        all_medicare_cols = ICD_diag_cols2 + HCPCS_proc_cols2 + ICD_procedure_cols2 + NDC_drug_cols2
        
        ######################################
        # check if all the columns exist
        ######################################
        all_medicare_cols_f = []
        for kkk in range(len(all_medicare_cols)):
            if all_medicare_cols[kkk] in medicare_columns_list:
                all_medicare_cols_f.append(all_medicare_cols[kkk]) 
        all_medicare_cols = all_medicare_cols_f
        ...
        cleaned_medicare_df = change_y_m_d(cleaned_medicare_df, 'medicare')
        ...
        if not cleaned_medicare_df.empty:
            ...
            raise ValueError("There are samples or records that do not belong to Medicare, please clean up the metadata file")
        ...
        for k in range(len(start_date)): #
            ...
            curr_unique_ICD_DIAG  = get_uniquecodes_perMonth("DIAG_ICD",df_empty,curr_month_df2,[],ICD_diag_cols2)
            curr_unique_ICD_PROC  = get_uniquecodes_perMonth("PROC_ICD",df_empty,curr_month_df2,[],ICD_procedure_cols2)
            curr_unique_HCPC_PROC = get_uniquecodes_perMonth("PROC_HCPCS",df_empty,curr_month_df2,[],HCPCS_proc_cols2)
            curr_unique_NDC_DRUG  = get_uniquecodes_perMonth("DRUG_NDC",df_empty,curr_month_df2,[],NDC_drug_cols2)
            curr_unique_AHFS_DRUG = get_uniquecodes_perMonth("DRUG_AHFS",df_empty,curr_month_df2,[],[])
