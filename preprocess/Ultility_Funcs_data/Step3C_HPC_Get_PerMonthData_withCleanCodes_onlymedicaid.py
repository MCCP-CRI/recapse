...
def Step3C_HPC_Get_PerMonthData_withCleanCodes_onlymedicaid(user_name, path_input, path_output, mecaidClaims, mecaidClaims2, drug_code):
    ...
    for i in range(len(analysis_ID)): #len(analysis_ID)
        ...
        #print(curr_id)
        data_res = read_allClaims2(curr_id, path_medicaid_heath, path_medicaid_pharm, medicaid_columns_list_health, medicaid_columns_list_pharm)
        ...
        #if not cleaned_medicaid_df.empty:
        #    unique_dates3 = cleaned_medicaid_df["claims_date"].unique()
        ...
        #if not cleaned_medicaid_health_df.empty and not cleaned_medicaid_pharm_df.empty and not cleaned_medicaid_df.empty:
        #    all_unique_dates = np.unique(np.concatenate((unique_dates1,unique_dates2,unique_dates3),axis=0))
        if not cleaned_medicaid_health_df.empty and cleaned_medicaid_pharm_df.empty:# and cleaned_medicaid_df.empty:
            all_unique_dates = np.unique(unique_dates1)
        elif cleaned_medicaid_health_df.empty and not cleaned_medicaid_pharm_df.empty:# and cleaned_medicaid_df.empty:
            all_unique_dates = np.unique(unique_dates2)
        #elif cleaned_medicaid_health_df.empty and cleaned_medicaid_pharm_df.empty and not cleaned_medicaid_df.empty:
        #    all_unique_dates = np.unique(unique_dates3)
        elif not cleaned_medicaid_health_df.empty and not cleaned_medicaid_pharm_df.empty:# and cleaned_medicaid_df.empty:
            all_unique_dates = np.unique(np.concatenate((unique_dates1,unique_dates2),axis=0))
        #elif not cleaned_medicaid_health_df.empty and cleaned_medicaid_pharm_df.empty and not cleaned_medicaid_df.empty:
        #    all_unique_dates = np.unique(np.concatenate((unique_dates1,unique_dates3),axis=0))
        #elif cleaned_medicaid_health_df.empty and not cleaned_medicaid_pharm_df.empty and not cleaned_medicaid_df.empty:
        ...
        for k in range(len(start_date)): #
            ...
            #curr_month_df2        = get_claims_inDateRange(cleaned_medicaid_df,"claims_date",mon, end)
