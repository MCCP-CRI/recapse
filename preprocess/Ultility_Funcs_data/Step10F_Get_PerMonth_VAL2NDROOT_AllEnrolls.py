...
def Step10F_Get_PerMonth_VAL2NDROOT_AllEnrolls(user_name, path_input, path_output):

    ###HPC
    path_column_code = str(path_output) + "/" +str(user_name) + '/10A_PerPatient_UniqueCodes'
    path_save = str(path_output) + "/" +str(user_name) + '/10F_VAL2NDFeature_inValidMonth'
    ...
    curr_df = drug_grp_df['VAL_SECONDARY_group']
    curr_df = curr_df.fillna('NA')
    curr_df = 'VAL_2ND_' + curr_df
    drug_grp_df['VAL_SECONDARY_group'] = curr_df
    drug_grp_df = drug_grp_df.rename(columns={'VAL_SECONDARY_group': code_type})
    ...
    for f in xlsx_files:
        ...
        if len(list(curr_df)) >3:
            ...
            if unique_codes_df.empty:
                ...
                curr_grp_feature_df = curr_df.iloc[:,0:3] #unique_grps_df.iloc[:,0:3] 
            else:
