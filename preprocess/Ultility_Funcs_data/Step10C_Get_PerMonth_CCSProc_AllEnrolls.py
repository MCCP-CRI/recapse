...
def Step10C_Get_PerMonth_CCSProc_AllEnrolls(user_name, path_input, path_output):
    ###HPC
    path_column_code = str(path_output) + "/" +str(user_name) + '/10A_PerPatient_UniqueCodes'
    path_save = str(path_output) + "/" +str(user_name) + '/10C_CCSProcFeature_inValidMonth'
    ...
    #reformat 
    code_type = 'CCS_PROC'
    proc_grp_df.loc[proc_grp_df['TYPE'] == 'PROC_ICD9or10', 'TYPE'] = "PROC_ICD"  #change code type name
    curr_df = proc_grp_df['CCS_CATEGORY']
    curr_df = curr_df.fillna('NA')
    curr_df = 'CCS_PROC_' + curr_df
    proc_grp_df['CCS_CATEGORY'] = curr_df
    proc_grp_df = proc_grp_df.rename(columns={'CCS_CATEGORY': code_type})
    ...
    for f in xlsx_files:
        ...
        if len(list(curr_df)) >3:
            ...
            frames = [curr_proc_ICD_codes, curr_proc_HCPCS_codes]
            ...
            if unique_codes_df.empty:
                ...
                unique_codes_df = find_listofcode_grp_func(unique_codes_df,code_type,proc_grp_df)   
