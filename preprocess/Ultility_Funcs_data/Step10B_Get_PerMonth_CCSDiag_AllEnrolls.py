...
def Step10B_Get_PerMonth_CCSDiag_AllEnrolls(user_name, path_input, path_output):

    ###HPC
    path_column_code = str(path_output) + "/" +str(user_name) + '/10A_PerPatient_UniqueCodes'
    path_save = str(path_output) + "/" +str(user_name) + '/10B_CCSDiagFeature_inValidMonth'
    ...
    curr_df = diag_grp_df['CCS_CATEGORY']
    curr_df = curr_df.fillna('NA')
    curr_df = 'CCS_DIAG_' + curr_df
    diag_grp_df['CCS_CATEGORY'] = curr_df
    diag_grp_df = diag_grp_df.rename(columns={'CCS_CATEGORY': code_type})
    ...
    for f in xlsx_files:
        ...
        if len(list(curr_df)) >3:
            ...
            if unique_codes_df.empty:
                ...
                unique_codes_df = find_listofcode_grp_func(unique_codes_df,code_type,diag_grp_df)   
