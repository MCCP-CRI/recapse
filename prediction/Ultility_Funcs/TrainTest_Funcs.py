...
def prediction(model,test_data,ID,cutoff,method = "Classifier"):
    #Prediction prob
    ...
    pred_prob_df = pd.DataFrame({'pred_prob': pred_prob})
...
def patient_level_prediction(prediction_df,threshold, pred_method = "3month"):
    ...
    for pt in patient_IDs:     
        ...
        #Load pt data
        #print(prediction_df)
        cur_pred_df = prediction_df[prediction_df['study_id'] == str(pt)].copy()
        #print('Here',cur_pred_df)
        #print('Here', label_df)
        #print(label_df.dtypes)
        ##cur_label_df = label_df[label_df['study_id'] == str(pt)].copy()
        #cur_label_df  = label_df.loc[label_df['study_id'] == str(pt)]
        #print('cur_label_df', cur_label_df)
        #print(cur_label_df)
    ...
    #Compute month diff
    #print("Here",pt_level_pred_df['pred_month'])
    #pt_level_pred_df['RAW_Month_Diff'] = (pt_level_pred_df['pred_month'])#/np.timedelta64(1, 'M') #- pt_level_pred_df['actual_month']
